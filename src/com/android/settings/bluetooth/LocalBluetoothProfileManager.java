...
public abstract class LocalBluetoothProfileManager {
    ...
    public static void updateProfiles(ParcelUuid[] uuids, List<Profile> profiles) {
        ...
        /* TI HID port - start */
        if (SystemProperties.OMAP_ENHANCEMENT &&
            BluetoothUuid.containsAnyUuid(uuids, HID_PROFILE_UUIDS)) {
    ...
    private static class A2dpProfileManager extends LocalBluetoothProfileManager {
        ...
        public boolean connect(BluetoothDevice device) {
            ...
            // Wait until the connections to the current sinks are disconnected
            // (up to 5 sec in case of some malfunctioning)
            int count = 0;
            boolean disconnected = false;
            while (!disconnected && (count < 50)) {
                disconnected = true;
                for (BluetoothDevice sink : sinks) {
                    if (mService.getSinkState(sink) != BluetoothA2dp.STATE_DISCONNECTED) {
                        disconnected = false;			
                        break;
                    }
                }
                if (!disconnected) {
		    Log.e(TAG, "Not all sinks are disconnected before new connection!!! " + count);
    ...
    private static class HeadsetProfileManager extends LocalBluetoothProfileManager
        ...
        public boolean connect(BluetoothDevice device) {
            ...
            // Wait until the connection to the current headset is disconnected
            // (up to 5 sec in case of some malfunctioning)
            int count = 0;
            while ((mService.getState() != BluetoothHeadset.STATE_DISCONNECTED) && (count < 50)) {
		Log.e(TAG, "We are not in STATE_DISCONNECTED before new connection!!! " + count);
    ...
    private static class HidProfileManager extends LocalBluetoothProfileManager {
        ...
        @Override
        public boolean connect(BluetoothDevice device) {
            if (SystemProperties.OMAP_ENHANCEMENT == false)
                return false;
